...
const Layout: React.FC = () => {
  ...
  const navItems = [
    ...
    { to: '/historico', icon: HistoryIcon, label: 'Extrato' },
    { to: '/categorias', icon: PieChart, label: 'Análise' },
    { to: '/faturas', icon: CreditCard, label: 'Cartões' },
    { to: '/metas', icon: Target, label: 'Caixinhas' },
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col xl:flex-row">
      ...
      <nav className="xl:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] bg-gray-900/95 backdrop-blur-md rounded-[2.5rem] px-4 py-3 flex items-center justify-between shadow-2xl z-50 border border-white/10">
        ...
        <NavLink 
          to="/novo" 
          className="bg-nubank-purple text-white w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-purple-600/30 -mt-12 border-4 border-[#F5F5F5] transition-transform active:scale-95 ring-4 ring-gray-900"
        >
          <PlusCircle size={24} />
        </NavLink>
